/**
 * Instance.js - Direct Element Architecture (v1.0.0-beta.5)
 * Base class for DOM components with zero-boilerplate inheritance: the element IS the class.
 */
...
class Instance {
    ...
    constructor() {
        ...
        /*  
            ...
            I had no real answer to a critical question that
            was in completely new territory:
            If a tab (lowercase 't') is both instanceof class
            AND element, what should tab['constructor'] return?

            I chose to leave it alone until I had figured it out, which I did:
                Constructors are, of course, objects, and the tab is an 
                instance of both constructors, so add both properties:
           ...
           you can opt out of default constructor property override in config,
           or per subclass by setting { constructor: false }
        */
        ...
        if (overrideConstructor) {
            const nativeConstructor = element.constructor;
        ...
        element.instance = this.constructor.name;
        
        // Technically metadata: Array-like interface (jQuery compatibility)
        element[0] = element;
        element.length = 1;
        ...
        // CRITICAL: Consturctor Return Override: Return element — not `this`
        // This makes `new Subclass()` → the DOM node
